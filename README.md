...
3. 在 **Table Editor** 中确认 `daily_records` 已创建。
...
`supabase/config.toml` 已为两个函数设置 `verify_jwt = false`，因为用户端没有 Supabase 登录态；两个函数会在代码中对 `BABY_ACCESS_TOKEN` 做 SHA-256 后的固定长度比较。数据库写入密钥只在 Edge Function 服务端环境读取，不会发给浏览器。
...
2. 用专属链接打开，确认 token 很快从地址栏消失。
3. 依次选择心情、身体、1—3 个需求，输入 100 字以内留言并提交。
4. 在 Supabase `daily_records` 确认当天记录存在，`viewed=false`。
5. 打开 `/admin.html`，分别测试错误密码和正确密码。
6. 确认工作台显示今天记录和对应的简单提醒。
7. 点击“我看到啦”，确认数据库出现 `viewed=true`、`viewed_at` 有值。
8. 返回用户提交成功页，最多等待 15 秒，确认变为“张先森已经看到了 💗”。
9. 当天重新提交，确认仍只有一条当天记录，内容更新且查看状态重置。
10. 打开 `/history.html`，确认最近七个自然日按倒序显示，无记录日期也存在。
11. 测试需求未选、选择超过三项、留言超过 100 字、非法接口数据，确认前后端都会拒绝。
12. 抽奖后刷新页面，确认当天不能再次抽首轮；若中奖，只允许换券一次。
13. 用 320px 宽度手机模拟器和桌面宽度检查页面，无横向滚动。
